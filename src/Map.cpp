...
void Map::draw() {
	...
	Coordinate tl = pointCoordinate(vec2(0)).zoomTo(baseZoom);
	Coordinate tr = pointCoordinate(vec2(size.x, 0)).zoomTo(baseZoom);
	Coordinate bl = pointCoordinate(vec2(0, size.y)).zoomTo(baseZoom);
...
//				}
	...
	gl::pushMatrices();
	gl::rotate(180.0*rotation / M_PI, 0, 0, 1);
	...
	for (citer = visibleKeys.begin(); citer != visibleKeys.end(); citer++) {
		...
		vec2 tileSize = mapProvider->getTileSize();
		tileSize *= scale;
		vec2 center = size;
		center *= 0.5;
		...
		if (images.count(coord) > 0) {
			gl::TextureRef tile = images[coord];
...
//			recentImages.erase(remove(recentImages.begin(), recentImages.end(), tile), recentImages.end());
	...
	if (recentImages.size() > numToKeep) {
		...
		std::map<Coordinate,gl::TextureRef>::iterator iter = images.begin();
		std::map<Coordinate, gl::TextureRef>::iterator endIter = images.end();
		for (; iter != endIter;) {
...
void Map::panBy(const vec2 &delta) { panBy(delta.x, delta.y); }
...
void Map::panBy(const double &dx, const double &dy) {
	...
	const vec2 tileSize = mapProvider->getTileSize();
...
void Map::scaleBy(const double &s, const vec2 &c) {
	scaleBy(s, c.x, c.y);
}
...
{
	return MapExtent(pointLocation(vec2(0)), pointLocation(size));
}
...
vec2 Map::coordinatePoint(const Coordinate &target) const
{
	...
	vec2 rotated(point);
	//BL rotated.rotate(rotation);
	
	return rotated;
}

Coordinate Map::pointCoordinate(const vec2 &point) const {
...
vec2 Map::locationPoint(const Location &location) const {
	return coordinatePoint(mapProvider->locationCoordinate(location));
}

Location Map::pointLocation(const vec2 &point) const {
