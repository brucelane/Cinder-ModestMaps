...
public:
	...
	void setTarget(const Location &l1, const double &z1, const double &_t0) {
		...
		c0 = vec2(start.column, start.row);
		c1 = vec2(end.column, end.row);
	...
	double visibleWorld() {
		// how much world can we see at zoom 0?
		Coordinate topLeft = map->pointCoordinate(vec2(0)).zoomTo(0);
	...
	void update(const double &t1) {
		...
		double us = u(s);
		//vec2 pos = lerp(c0, c1, (us - u0) / (u1 - u0));
		vec2 pos = c0;
