...
public:
	...
	vec2 transform(const vec2 &point) {
		return vec2(ax*point.x + bx*point.y + cx, ay*point.x + by*point.y + cy);
	}
	
	vec2 untransform(const vec2 &point){
		return vec2((point.x*by - point.y*bx - cx*by + cy*bx) / (ax*by - ay*bx), (point.x*ay - point.y*ax - cx*ay + cy*ax) / (bx*ay - by*ax));
	}
    ...
    static Transformation deriveTransformation( const double &a1x, const double &a1y, 
		...
		const vec3 d = linearSolution(a1x, a1y, a2x,
                                        b1x, b1y, b2x,
                                        c1x, c1y, c2x );
		const vec3 e = linearSolution(a1x, a1y, a2y,
                                        b1x, b1y, b2y,
                                        c1x, c1y, c2y );
    ...
    static vec3 linearSolution( const double &r1, const double &s1, const double &t1, 
